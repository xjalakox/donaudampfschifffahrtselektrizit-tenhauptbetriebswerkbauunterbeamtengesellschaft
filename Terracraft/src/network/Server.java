	...
	protected void parsePacket(byte[] data, InetAddress address, int port) {
		...
		case MYSQL_REGISTER:
			...
			if (mysql.checkIfUsernameTaken(packet04.getUsername())) {
				...
				super.send(new Packet06Message("Du hast dich erfolgreich registriert und kannst dich nun anmelden!").getData(), user3);
			} else {
				super.send(new Packet06Message("Der angegeben Username ist bereits benutzt! Suche dir einen anderen aus.").getData(), user3);
			}
		}
	}
